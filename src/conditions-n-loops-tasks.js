...
function convertToRomanNumerals(num) {
  ...
  for (let i = 0; i < romanMatrix.length; i += 1) {
    if (num >= romanMatrix[i][0]) {
      return (
        romanMatrix[i][1] + convertToRomanNumerals(num - romanMatrix[i][0])
      );
    }
  }
...
function convertNumberToString(numberStr) {
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    const isLastIndex = i === numberStr.length - 1;
    const shouldSpace = numberStr.length > 1 && !isLastIndex;
    switch (numberStr[i]) {
      default:
        result += `${wordsMap[numberStr[i]]}${shouldSpace ? ' ' : ''}`;
        break;
    }
  }
